...
object StreamBarrier {
  ...
  def apply[T1 <: Data, T2 <: Data, T3 <: Data](
    ...
    barrierFire := (rFire(0) && rFire(1) && src2.fire) || (rFire(1) && rFire(2) && src0.fire) || (rFire(0) && rFire(
      2
    ) && src1.fire) ||
      (src0.fire && src1.fire && rFire(2)) || (src1.fire && src2.fire && rFire(0)) || (src0.fire && src2.fire && rFire(
        1
      )) ||
